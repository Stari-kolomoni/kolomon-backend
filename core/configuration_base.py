"""
Handles entire configuration for the Stari Kolomoni project. Uses toml configuration files (with tomli).
"""
...
# Points to the main directory.
BASE_PROJECT_DIR = Path(os.path.dirname(__file__), "..").absolute()
...
def _get_value_with_placeholders(
    ...
    if configuration_value is None:
        if allow_empty:
            ...
            raise ConfigurationException(f"Missing configuration value: {key}")
    ...
    if configuration_value.strip(" ") == "":
        if allow_empty:
            ...
            raise ConfigurationException(f"Value at {key} is empty (only whitespace).")
...
def _get_str_list_value(table: "TOMLConfig", key: str) -> list[str]:
    ...
    if not isinstance(configuration_value, list):
        raise ConfigurationException(f"{key} was expected to be list[str], got {type(configuration_value)}")
...
class TOMLConfig:
    ...
    def get_table(self, name: str, raise_on_missing_key: bool = False) -> Optional["TOMLConfig"]:
        ...
        if data is None:
            if raise_on_missing_key:
                raise ConfigurationException(f"Configuration table missing: '{name}'")
            else:
                return None
    ...
    def get(self, name: str, fallback: Any = None, raise_on_missing_key: bool = False) -> Any:
        ...
        if data is None and raise_on_missing_key:
            raise ConfigurationException(f"Configuration value missing: '{name}'")
        elif data is None:
            return fallback
        else:
            return data
