...
def get_search_quick(db: Session, pagination: Pagination, term: str) -> List[schemas.QuickSearchResult]:
    ...
    s1 = select([en.id, en.lemma, en.description]).filter(en.lemma.ilike('%' + term + '%')).add_columns(bindparam('lang', 'en').label('language'))

    sl = models.SloveneEntry
    s2 = select([sl.id, sl.lemma, sl.description]).filter(sl.lemma.ilike('%' + term + '%')).add_columns(bindparam('lang', 'sl').label('language'))
...
def get_search_full(db: Session, pagination: Pagination, term: str) -> List[schemas.FullSearchResult]:
    sql = """
        ...
        FULL OUTER JOIN slovene_entries sl
        ON en.translation_id = sl.id
